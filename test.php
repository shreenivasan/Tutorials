...
$dsn = "mysql:host=localhost;dbname=temp_db;user=root;password=";

$db = new PDO($dsn, $username, $password);
$db->setAttribute(PDO::ATTR_DEFAULT_FETCH_MODE, PDO::FETCH_OBJ);

$lat = !empty($_GET['lat']) ? $_GET['lat'] : "31.6434812"; // latitude of centre of bounding circle in degrees
$lon = !empty($_GET['lon']) ? $_GET['lon'] : "74.8300599"; // longitude of centre of bounding circle in degrees
$rad = !empty($_GET['rad']) ? $_GET['rad'] : "5"; // radius of bounding circle in kilometers
...
foreach ($points as $point) {
    ...
    ?>
            <tr>
                <td><?php echo $point->pincode ?></td>
                <td><?php echo number_format($point->latitude, 4); ?></td>
                <td><?php echo number_format($point->longitude, 4); ?></td>
                <td><?php echo number_format($point->D, 3); ?></td>
...
$strColumn = "";
foreach ($data["header"] as $k) {
    $strColumn .= "" . $k . ",";
}
$strColumn = rtrim($strColumn, ",");

$lSql = "insert into stores($strColumn) VALUES";
$sql = '';
