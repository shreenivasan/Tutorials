...
class Purchasedetails extends CI_Controller 
{
	...
	$this->data['ascDesc']	= $ascDesc;
        ...
        if ((isset($_POST['txtSearch']) || ($this->input->post('hdnSearch') == '1')) || $search!="") {
            ...
            $searchData=$this->Purchasedetails_model->fetch_rows($this->current_module,$config["per_page"], $page,$selectCriteria, $sortBy, $sortOrder,$searchval);             
            ...
            $respData = array();
            $respData=$this->Purchasedetails_model->fetch_rows($this->currentModule,$config["per_page"], $page,$selectCriteria);
...
;
    ...
    {      
        $this->data['mode'] = 'Add'; 
        $this->data['product_details']=$this->Purchasedetails_model->get_product_details();
        $this->data['seller_details']=  $this->Purchasedetails_model->get_seller_details();
    ...
    {      
        ...
        $this->data['product_details']=$this->Purchasedetails_model->get_product_details();
        $this->data['seller_details']=  $this->Purchasedetails_model->get_seller_details();
        $this->data["po_details"] = $this->Purchasedetails_model->get_purchase($id,$this->tableid);        
        $this->data["po_details"]=  array_shift($this->data["po_details"]);        
        $this->data["pod_details"] = $this->Purchasedetails_model->get_purchase_details($id);
        $this->load->view($this->view_dir_name.'/EditView.phtml', $this->data);
    }
    ...
    {   
        ...
        if($postData['id']=="")
        {
            ...
            else
            {
                ...
                {
                    redirect(site_url($this->currentModule) . "/index");
                }
            }
        ...
        else
        {
            ...
            if($this->db->update($this->table_name, $data))
            {
                ...
                for($i=0; (isset($postData['product']) && $i<count($postData['product']) );$i++){
                ...
                if(count($purchase_details_array)>0)
                {
                    $this->db->insert_batch("purchase_order_details",$purchase_details_array);
                    $inserted_id2= $this->db->insert_id();
                }
